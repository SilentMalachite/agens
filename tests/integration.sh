...
if echo "$out" | grep -q "Apple Silicon"; then
    ...
    ok "invalid ratio clamped on Apple Silicon"
else
    ok "invalid ratio clamped - skipped on non-Apple Silicon"
fi
